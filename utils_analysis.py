...
def plotComparisonShapleyValues(
    ...
    if sort == "alp":
        sh_score_1 = { k: v*100 for k, v in sorted(sh_score_1.items(), key=lambda item: item[0], reverse=True) }
        sh_score_2 = { k: v*100 for k, v in sorted(sh_score_2.items(), key=lambda item: item[0], reverse=True) }

    if sort == "rev_alp":
        sh_score_1 = { k: v*100 for k, v in sorted(sh_score_1.items(), key=lambda item: item[0]) }
        sh_score_2 = { k: v*100 for k, v in sorted(sh_score_2.items(), key=lambda item: item[0]) }
